...
// // Show the Add Student tab by default
// showTab('addStudent');
...
//     const selectedCourses = Array.from(document.querySelectorAll('#courseCheckboxes input:checked')).map(checkbox => checkbox.value);
...
//     const studentId = Date.now(); // Use timestamp as a unique ID
...
//             // Check if the selected course is in the student's courses array
//             if (student.courses && student.courses.includes(courseSelect)) {
//                 const studentItem = document.createElement('div');
//                 studentItem.innerHTML = `<input type="checkbox" id="${childSnapshot.key}"> ${student.name}`;
...
//         const studentId = checkbox.id;
//         const attendanceRef = ref(db, 'attendance/' + studentId);
//         set(attendanceRef, {
//             present: true,
//             timestamp: Date.now()
//         }).then(() => {
//             alert('Attendance marked for ' + checkbox.nextSibling.textContent);
//         }).catch((error) => {
...
// document.getElementById('addStudentTab').addEventListener('click', () => showTab('addStudent'));
...
//         { time: "18:00-19:00", course: "Art" },
//         { time: "19:00-20:00", course: "History" }
//     ],
//     Saturday: [
//         { time: "10:00-11:00", course: "History" },
//         { time: "11:00-12:00", course: "Literature" },
//         { time: "12:00-13:00", course: "Art" },
//         { time: "18:00-19:00", course: "Science" },
//         { time: "19:00-20:00", course: "Mathematics" }

//     ],
//     Friday: [
//         { time: "11:00-12:00", course: "Science" },
//         { time: "12:00-13:00", course: "Mathematics" },
//         { time: "17:00-18:00", course: "Literature" },
//         { time: "18:00-19:00", course: "Art" },
//         { time: "19:00-20:00", course: "History" }
//     ]
...
//     // Initialize attendance for each course
//     courses.forEach(course => {
//         newStudent.attendance[course] = [];
//     });
...
//             if (student.courses && student.courses.includes(courseSelect)) {
//                 const studentItem = document.createElement('div');
//                 studentItem.innerHTML = `<input type="checkbox" id="${childSnapshot.key}" data-course="${courseSelect}"> ${student.name}`;
//                 studentList.appendChild(studentItem);
//             }
//         });
//     });
...
//         // Check if the attendance object for the course exists
//         get(studentRef).then((snapshot) => {
//             if (snapshot.exists()) {
//                 const studentData = snapshot.val();
//                 const attendanceKey = `attendance/${course}`;

//                 // If the attendance array doesn't exist, initialize it
//                 if (!studentData.attendance || !studentData.attendance[course]) {
//                     update(studentRef, {
//                         [attendanceKey]: [currentDate]
//                     }).then(() => {
...
//                     update(studentRef, {
//                         [attendanceKey]: firebase.database.ServerValue.arrayUnion(currentDate)
//                     }).then(() => {
//                         alert('Attendance marked for ' + checkbox.nextSibling.textContent + ' in ' + course);
//                     }).catch((error) => {
//                         console.error('Error marking attendance: ', error);
...
//             if (snapshot.exists()) {
//                 const studentData = snapshot.val();
//                 const attendanceArray = studentData.attendance[course] || [];
